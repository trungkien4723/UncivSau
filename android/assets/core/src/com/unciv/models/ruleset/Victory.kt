...
class Milestone(val uniqueDescription: String, private val parentVictory: Victory) {
    ...
    fun getMoreCountableThanOtherCivPercent(civ: Civilization, otherCiv: Civilization): Float {
        val countable1: Int
        val countable2: Int
        if (Stat.safeValueOf(params[0]) == Stat.Tourism && Stat.safeValueOf(params[1]) == Stat.Culture) {
            // Civ VI style: accumulated Tourism vs accumulated Culture
            countable1 = civ.storedTourism
            countable2 = otherCiv.totalCultureForContests
        } else {
            countable1 = Countables.getCountableAmount(params[0], GameContext(civ)) ?: 0
            countable2 = Countables.getCountableAmount(params[1], GameContext(otherCiv)) ?: 0
        }
