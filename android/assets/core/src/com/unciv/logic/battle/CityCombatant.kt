...
override fun getUnitType(): UnitType = UnitType.City
    override fun getAttackingStrength(defender: ICombatant?, combatAction: CombatAction): Int = (getCityStrength(defender, combatAction) * 0.75).roundToInt()
    @Readonly override fun getDefendingStrength(attacker: ICombatant?, combatAction: CombatAction): Int {
        if (isDefeated()) return 1
        return getCityStrength(attacker, combatAction)
    }
