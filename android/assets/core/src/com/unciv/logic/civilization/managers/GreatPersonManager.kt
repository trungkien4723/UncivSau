...
class GreatPersonManager : IsPartOfGameInfoSerialization {
    ...
    var pointsForNextGreatAdmiral = 200
    var pointsForNextGreatAdmiralCounter = Counter<String>()

    var greatPersonPointsCounter = Counter<String>()
    var greatGeneralPointsCounter = Counter<String>()
    var greatAdmiralPointsCounter = Counter<String>()
    ...
    fun clone(): GreatPersonManager {
        ...
        toReturn.pointsForNextGreatAdmiralCounter = pointsForNextGreatAdmiralCounter.clone()
        toReturn.greatAdmiralPointsCounter = greatAdmiralPointsCounter.clone()
    ...
    fun getPointsRequiredForGreatGeneral(greatGeneral: String): Int {
        if (pointsForNextGreatGeneralCounter[greatGeneral] == 0) {
            pointsForNextGreatGeneralCounter[greatGeneral] = 200
        }
        return (pointsForNextGreatGeneralCounter[greatGeneral] * civInfo.gameInfo.speed.modifier).toInt()
    }

    fun getPointsRequiredForGreatAdmiral(greatAdmiral: String): Int {
        if (pointsForNextGreatAdmiralCounter[greatAdmiral] == 0) {
            pointsForNextGreatAdmiralCounter[greatAdmiral] = 200
        }
        return (pointsForNextGreatAdmiralCounter[greatAdmiral] * civInfo.gameInfo.speed.modifier).toInt()
    }

    fun getNewGreatPerson(): String? {
        for ((unit, value) in greatGeneralPointsCounter) {
            val requiredPoints = getPointsRequiredForGreatGeneral(unit)
        ...
        for ((unit, value) in greatAdmiralPointsCounter) {
            val requiredPoints = getPointsRequiredForGreatAdmiral(unit)
            if (value > requiredPoints) {
                greatAdmiralPointsCounter[unit] -= requiredPoints
                pointsForNextGreatAdmiralCounter[unit] += 50
                return unit
            }
        }
    ...
    fun addGreatGeneralPoints(points: Counter<String>) {
        greatGeneralPointsCounter.add(points)
    }

    fun addGreatAdmiralPoints(points: Counter<String>) {
        greatAdmiralPointsCounter.add(points)
    }
