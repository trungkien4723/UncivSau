...
class DiplomacyFunctions(val civInfo: Civilization) {
    ...
    // === Casus Belli Availability Checks (Civ VI Style) ===
    ...
    @Readonly
    fun canDeclareHolyWar(otherCiv: Civilization): Boolean {
        // Can declare holy war if we have founded a religion and they have a different religion
        return civInfo.religionManager.religion != null &&
               otherCiv.religionManager.religion != null &&
               civInfo.religionManager.religion != otherCiv.religionManager.religion
    }

    @Readonly
    fun canDeclareLiberationWar(otherCiv: Civilization): Boolean {
        // Can declare liberation war if they hold a city that originally belonged to us or an ally
        return otherCiv.cities.any { city ->
            city.foundingCivObject == civInfo || 
            (city.foundingCivObject != null && city.foundingCivObject!!.getDiplomacyManager(civInfo)?.diplomaticStatus == DiplomaticStatus.DefensivePact == true)
        }
    }

    @Readonly
    fun canDeclareReconquestWar(otherCiv: Civilization): Boolean {
        // Can declare reconquest war if they hold a city that originally belonged to us
        return otherCiv.cities.any { city -> city.foundingCivObject == civInfo }
    }

    @Readonly
    fun canDeclareProtectorateWar(otherCiv: Civilization): Boolean {
        // Can declare protectorate war if they attacked our city-state ally
        return civInfo.diplomacy.values.any { diplo ->
            diplo.otherCiv.isCityState && diplo.otherCiv.allyCiv == civInfo &&
            diplo.otherCiv.isAtWarWith(otherCiv)
        }
    }

    @Readonly
    fun canDeclareColonialWar(otherCiv: Civilization): Boolean {
        // Can declare colonial war if they are on a different continent
        val ourCapital = civInfo.getCapital()
        val theirCapital = otherCiv.getCapital()
        return ourCapital != null && theirCapital != null &&
               ourCapital.getCenterTile().getContinent() != theirCapital.getCenterTile().getContinent()
    }
