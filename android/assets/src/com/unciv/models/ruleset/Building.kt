...
class Building : RulesetStatsObject(), INonPerpetualConstruction {
    ...
    override fun getProductionCost(civInfo: Civilization, city: City?): Int {
        ...
        // Civ VI district cost scaling: cost increases with tech/civic progress (Gathering Storm formula)
        // cost = base_cost * (1 + 9 * max(techProgress, civicProgress))
        if (hasCreateOneDistrictUnique() && city != null) {
            val totalTechs = civInfo.gameInfo.ruleset.technologies.size
            val totalCivics = civInfo.gameInfo.ruleset.civics.size
            if (totalTechs > 0 && totalCivics > 0) {
                val techProgress = civInfo.tech.getNumberOfTechsResearched().toFloat() / totalTechs
                val civicProgress = civInfo.civics.getNumberOfCivicsResearched().toFloat() / totalCivics
                val scalingFactor = 1f + 9f * maxOf(techProgress, civicProgress)
                productionCost *= scalingFactor
            }
        }
