...
class CityScreenTileTable(private val cityScreen: CityScreen) : Table() {
    ...
    fun update(selectedTile: Tile?) {
        ...
        val pickData = cityScreen.pickDistrictData
        if (pickData != null) {
            val adjacency = city.cityStats.getDistrictAdjacencyStats(selectedTile, pickData.district)
            if (!adjacency.isEmpty()) {
                innerTable.add("Adjacency bonus:".toLabel()).padTop(3f).colspan(2).row()
                innerTable.add(getTileStatsTable(adjacency)).row()
            }
        }
