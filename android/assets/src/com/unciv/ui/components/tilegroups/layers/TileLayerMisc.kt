...
class TileLayerImprovement(tileGroup: TileGroup, size: Float) : TileLayer(tileGroup, size){
    ...
    override fun doUpdate(viewingCiv: Civilization?) {
        ...
        updateDistrictIcon(true) // always show district icons
    }
    ...
    private fun updateDistrictIcon(show: Boolean) {
        ...
        val isUnderConstruction = tile.district == null && tile.districtToCreate != null
        val newDistrictID = if (districtName == null) null
        else if (isPillaged) "$districtName-Pillaged"
        else if (isUnderConstruction) "$districtName-Construction"
        else districtName
        ...
        if (districtName != null && show && districtIcon == null) {
            val icon = ImageGetter.getConstructionPortrait(districtName, 24f)
            icon.x = tileX + (tileGroup.width - icon.width) / 2 + 22f
            icon.y = tileY + (tileGroup.height - icon.height) / 2 - 12f
            if (isUnderConstruction) icon.color.a = 0.5f
            else if (isPillaged) icon.color.a = 0.5f
            addOwnedActor(icon)
            districtIcon = icon
        }
