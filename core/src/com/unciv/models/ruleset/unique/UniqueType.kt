...
enum class UniqueType(
    ...
    // Placed here (before Stats) deliberately: Stats' placeholderText is "[]", and since UniqueType.uniqueTypeMap
    // uses last-wins associateBy, Stats must be the last entry whose placeholder collapses to "[]" so "[+1 Culture]"
    // style stat uniques keep parsing as Stats. These two Civ VI entries therefore use distinct placeholder text.
    ...
    GainEnvoy("Gain [amount] Envoys per turn", UniqueTarget.Global, UniqueTarget.Unit, UniqueTarget.Promotion),
...
OneTimeUnitGetsName("[unitTriggerTarget] gets a name from the [unitNameGroup] group", UniqueTarget.UnitTriggerable),
    ...
    TriggerUponEnteringDarkAge("upon entering a Dark Age", UniqueTarget.TriggerCondition),
    TriggerUponEnteringHeroicAge("upon entering a Heroic Age", UniqueTarget.TriggerCondition),
    ...
    //region Civ VI Eureka / modifier conditionals
    Civ6TriggerUponFindingTile("upon finding a [tileFilter]", UniqueTarget.TriggerCondition, UniqueTarget.UnitTriggerCondition),
    Civ6TriggerUponImprovingResourceWithImprovement("upon improving a resource with a [improvementFilter]", UniqueTarget.TriggerCondition, UniqueTarget.UnitTriggerCondition),
    Civ6TriggerUponMeetingCivilization("upon meeting another civilization", UniqueTarget.TriggerCondition, UniqueTarget.UnitTriggerCondition),
    Civ6TriggerUponImprovingTilesWithImprovement("upon improving [positiveAmount] tiles with [improvementFilter]", UniqueTarget.TriggerCondition, UniqueTarget.UnitTriggerCondition),
    Civ6TriggerUponConstructingBuilding("upon constructing a [buildingFilter]", UniqueTarget.TriggerCondition, UniqueTarget.UnitTriggerCondition),
    Civ6TriggerUponConstructingUnit("upon constructing a [baseUnitFilter] unit", UniqueTarget.TriggerCondition, UniqueTarget.UnitTriggerCondition),
    Civ6TriggerUponKillingUnitWith("upon killing a unit with a [baseUnitFilter]", UniqueTarget.TriggerCondition, UniqueTarget.UnitTriggerCondition),
    Civ6TriggerUponBuildingBuilding("upon building a [buildingFilter]", UniqueTarget.TriggerCondition, UniqueTarget.UnitTriggerCondition),
    Civ6TriggerUponBuildingDistrict("upon building a [districtFilter] district", UniqueTarget.TriggerCondition, UniqueTarget.UnitTriggerCondition),
    Civ6TriggerUponConstructingBuildingSuffix("upon constructing a [param] building", UniqueTarget.TriggerCondition, UniqueTarget.UnitTriggerCondition),
    Civ6TriggerUponBuildingBuildingSuffix("upon building a [param] building", UniqueTarget.TriggerCondition, UniqueTarget.UnitTriggerCondition),
    Civ6TriggerUponBuildingImprovementOnResource("upon building a [improvementFilter] on a [param] resource", UniqueTarget.TriggerCondition, UniqueTarget.UnitTriggerCondition),
    Civ6TriggerUponConstructingImprovement("upon constructing a [improvementFilter] improvement", UniqueTarget.TriggerCondition, UniqueTarget.UnitTriggerCondition),
    Civ6ForUnitsWorkingTile("for [baseUnitFilter] units working this tile", UniqueTarget.Conditional),
    //endregion
    ...
    // region CIV VI DESCRIPTION AND REMAINING STAT UNIQUES
    Civ6ModernEraCounterToCavalryClassUnits("Modern era counter to cavalry class units.", UniqueTarget.Unit),
    Civ6AtomicEraMeleeNavalUnitThatCanTransportAirUnits("Atomic era melee naval unit that can transport air units.", UniqueTarget.Unit),
    Civ6AntiAirStrength90("90Anti-air Strength", UniqueTarget.Unit),
    Civ6ModernEraAntiAirSupportUnitProvidesCoverFromAirAttacksUpTo1HexAwayFromTheWeapon("Modern era anti-air support unit. Provides cover from air attacks up to 1 hex away from the weapon.", UniqueTarget.Unit),
    Civ6AdjacentEnemyReligiousUnitsReceive10ReligiousStrengthDuringTheologicCombatReligiousUnit("Adjacent enemy religious units receive -10Religious Strength during Theologic Combat. Religious Unit", UniqueTarget.Unit),
    Civ6MayEvangelizeBeliefs("May Evangelize Beliefs", UniqueTarget.Unit),
    Civ6MayLaunchInquisition("May Launch Inquisition", UniqueTarget.Unit),
    Civ6MayConvertCitizensToTheirReligionCanOnlySpreadReligionInCitiesThatDoNotHaveAMajorityReligionReligionSpreadsInlandFromCoastalCitiesCanSpreadReligion("May convert Citizens to their Religion. Can only Spread Religion in cities that do not have a majority Religion. Religion spreads inland from coastal cities. Can Spread Religion.", UniqueTarget.Unit),
    Civ6ReligiousUnitsCanEnterOtherCivilizationsTerritoryWithoutAnOpenBorderAgreement("Religious units can enter other civilizations' territory without an open border agreement.", UniqueTarget.Unit),
    Civ6FirstAncientEraRangedUnitWithARangeOf2("First Ancient era ranged unit with aRange of 2.", UniqueTarget.Unit),
    Civ6BombardStrength80("80Bombard Strength", UniqueTarget.Unit),
    Civ6ModernEraSiegeUnitIdealForAttackingCitiesCannotMoveAndAttackOnTheSameTurnUnlessTheyVeEarnedTheExpertCrewPromotion("Modern era siege unit, ideal for attacking cities. Cannot move and attack on the same turn unless they've earned the Expert Crew promotion.", UniqueTarget.Unit),
    Civ6AncientEraSupportUnitWhenAdjacentToACityAttackingMeleeAndAntiCavalryUnitsDoFullDamageToTheCitySWallsDoesNotAffectCitiesWithMedievalOrRenaissanceWallsIneffectiveAgainstUrbanDefenses("Ancient era support unit. When adjacent to a city, attacking melee and anti-cavalry units do full damage to the city’s Walls. Does not affect cities with Medieval or Renaissance Walls. Ineffective against Urban Defenses.", UniqueTarget.Unit),
    Civ6ModernEraNavalRangedUnitWithRangeOf3ProvidesCoverFromAirAndNuclearAttacksUpTo1TileAway("Modern era naval ranged unit withRange of 3. Provides cover from air and nuclear attacks up to 1 tile away.", UniqueTarget.Unit),
    Civ6FirstAirCombatUnitAvailableInTheModernEra("First air combat unit, available in the Modern era.", UniqueTarget.Unit),
    Civ6BombardStrength55("55Bombard Strength", UniqueTarget.Unit),
    Civ6RenaissanceEraSiegeUnitIdealForAttackingCitiesCannotMoveAndAttackOnTheSameTurnUnlessTheyVeEarnedTheExpertCrewPromotion("Renaissance era siege unit, ideal for attacking cities. Cannot move and attack on the same turn unless they've earned the Expert Crew promotion.", UniqueTarget.Unit),
    Civ6BombardStrength105("105Bombard Strength", UniqueTarget.Unit),
    Civ6FirstBomberUnitAvailableInTheAtomicEra("First bomber unit, available in the Atomic era.", UniqueTarget.Unit),
    Civ6FastRenaissanceEraExplorationMeleeNavalUnit("Fast Renaissance era exploration melee naval unit.", UniqueTarget.Unit),
    Civ6BombardStrength35("35Bombard Strength", UniqueTarget.Unit),
    Civ6ClassicalEraSiegeUnitIdealForAttackingCitiesCannotMoveAndAttackOnTheSameTurnUnlessTheyVeEarnedTheExpertCrewPromotion("Classical era siege unit, ideal for attacking cities. Cannot move and attack on the same turn unless they've earned the Expert Crew promotion.", UniqueTarget.Unit),
    Civ6MedievalEraLightCavalryUnit("Medieval Era light cavalry unit.", UniqueTarget.Unit),
    Civ6MedievalEraRangedUnit("Medieval era ranged unit.", UniqueTarget.Unit),
    Civ6IndustrialEraHeavyCavalryUnit("Industrial Era heavy cavalry unit.", UniqueTarget.Unit),
    Civ6StrongestMeleeNavalUnitRevealsNavalRaidersWithinSightRange("Strongest melee naval unit. Reveals Naval Raiders within sight range.", UniqueTarget.Unit),
    Civ6BombardStrength50("50Bombard Strength", UniqueTarget.Unit),
    Civ6AtomicEraSupportUnitAdds1RangeAnd5BombardStrengthToSiegeClassUnitsWithin1Hex("Atomic era support unit. Adds +1Range and +5Bombard Strength to Siege class units within 1 hex.", UniqueTarget.Unit),
    Civ6IndustrialEraRangedUnit("Industrial era ranged unit.", UniqueTarget.Unit),
    Civ6AtomicEraBiplaneUpgrade("Atomic era Biplane upgrade.", UniqueTarget.Unit),
    Civ6RenaissanceEraRangedNavalUnitWithRangeOf2("Renaissance era ranged naval unit withRange of 2.", UniqueTarget.Unit),
    Civ6AncientEraMeleeNavalCombatUnitCanOnlyOperateOnCoastalWatersUntilCartographyIsResearched("Ancient era melee naval combat unit. Can only operate on Coastal waters until Cartography is researched.", UniqueTarget.Unit),
    Civ6AntiAirStrength120("120Anti-air Strength", UniqueTarget.Unit),
    Civ6InformationEraSuperUnitAbleToMoveAndFightInWaterResistantToWeaponsOfMassDestructionCombatAbilitiesCanBeUpgradedByResearchingFutureTechnologies("Information Era Super-Unit. Able to move and fight in water. Resistant to Weapons of Mass Destruction. Combat abilities can be upgraded by researching future technologies.", UniqueTarget.Unit),
    Civ6HealCharges3("3Heal Charges", UniqueTarget.Unit),
    Civ6MayUseAChargeToHealAdjacentReligiousUnitsBy30HPAndExtraBy5HPPerTheologicalCombatVictory("May use a charge to heal adjacent religious units by 30HP and extra by 5HP per Theological Combat victory", UniqueTarget.Unit),
    Civ6FastMovingAtomicEraLightCavalryUnit("Fast-moving Atomic era light cavalry unit.", UniqueTarget.Unit),
    Civ6FastMovingClassicalEraLightCavalryUnit("Fast-moving Classical era light cavalry unit.", UniqueTarget.Unit),
    Civ6ModernEraMeleeUnit("Modern era melee unit.", UniqueTarget.Unit),
    Civ6MayReduceThePresenceOfOtherReligionsInCitiesWhereTheMajorityReligionIsReligionFromCityStateW("May reduce the presence of other Religions in cities where the majority Religion is: Religion from city state w", UniqueTarget.Unit),
    Civ6PowerfulIndustrialEraArmoredMeleeNavalUnit("Powerful Industrial era armored melee naval unit.", UniqueTarget.Unit),
    Civ6BombardStrength115("115Bombard Strength", UniqueTarget.Unit),
    Civ6InformationEraBomberUpgrade("Information era Bomber upgrade.", UniqueTarget.Unit),
    Civ6InformationEraFighterUpgrade("Information era Fighter upgrade.", UniqueTarget.Unit),
    Civ6HardHittingMedievalEraHeavyCavalryUnit("Hard-hitting, Medieval era heavy cavalry unit.", UniqueTarget.Unit),
    Civ6BuildCharges1("1Build Charges", UniqueTarget.Unit),
    Civ6IndustrialEraMeleeUnit("Industrial Era melee unit.", UniqueTarget.Unit),
    Civ6AtomicEraRangedUnit("Atomic era ranged unit.", UniqueTarget.Unit),
    Civ6MedievalEraMeleeUnit("Medieval Era melee unit.", UniqueTarget.Unit),
    Civ6InformationEraMeleeUnit("Information era melee unit.", UniqueTarget.Unit),
    Civ6AntiAirStrength95("95Anti-air Strength", UniqueTarget.Unit),
    Civ6AntiAirStrength110("110Anti-air Strength", UniqueTarget.Unit),
    Civ6InformationEraRangedNavalUnitWithRangeOf4ProvidesCoverFromAirAndNuclearAttacksUpTo1TileAway("Information era ranged naval unit withRange of 4. Provides cover from air and nuclear attacks up to 1 tile away.", UniqueTarget.Unit),
    Civ6MayConvertCitizensToTheirReligionCanOnlySpreadReligionInCitiesThatDoNotHaveAMajorityReligionReligionSpreadsInlandFromCoastalCitiesCanSpreadReligionReligiousUnitsCanEnterOtherCivilizationsTerritoryWithoutAnOpenBorderAgreement("May convert Citizens to their Religion. Can only Spread Religion in cities that do not have a majority Religion. Religion spreads inland from coastal cities. Can Spread Religion. Religious units can enter other civilizations' territory without an open border agreement.", UniqueTarget.Unit),
    Civ6InformationEraAntiAirSupportUnitProvidesCoverFromAirAndNuclearAttacksUpTo1HexAwayFromTheWeapon("Information era anti-air support unit. Provides cover from air and nuclear attacks up to 1 hex away from the weapon.", UniqueTarget.Unit),
    Civ6InformationEraCounterToCavalryClassUnits("Information era counter to cavalry class units.", UniqueTarget.Unit),
    Civ6InformationEraHeavyCavalryUnitThatIsFastAndStrong("Information era heavy cavalry unit that is fast and strong.", UniqueTarget.Unit),
    Civ6RenaissanceEraMeleeUnit("Renaissance era melee unit.", UniqueTarget.Unit),
    Civ6BuildCharges2("2Build Charges", UniqueTarget.Unit),
    Civ6HighlyAdvancedInformationEraRangedRaidingNavalUnitThatCanBeArmedWithNuclearWeaponsCanOnlyBeSeenByOtherNavalRaidersUnlessAdjacentToItRevealsNavalRaidersWithinSightRange("Highly advanced Information era ranged raiding naval unit that can be armed with nuclear weapons. Can only be seen by other Naval Raiders unless adjacent to it. Reveals Naval Raiders within sight range.", UniqueTarget.Unit),
    Civ6ModernEraSupportUnitGrants1RangeToSiegeClassUnitsWithin1Hex("Modern era support unit. Grants +1Range to Siege class units within 1 hex.", UniqueTarget.Unit),
    Civ6LateRenaissanceEraAntiCavalryUnit("Late Renaissance era anti-cavalry unit.", UniqueTarget.Unit),
    Civ6MedievalEraAntiCavalryUnit("Medieval era anti-cavalry unit.", UniqueTarget.Unit),
    Civ6RenaissanceEraRangedNavalUnitWithTheAbilityToCoastalRaidCanOnlyBeSeenByOtherNavalRaidersUnlessAdjacentToItRevealsNavalRaidersWithinSightRange("Renaissance era ranged naval unit with the ability to coastal raid.Can only be seen by other Naval Raiders unless adjacent to it. Reveals Naval Raiders within sight range.", UniqueTarget.Unit),
    Civ6ClassicalEraRangedNavalUnitWithRangeOf1CanOnlyOperateOnCoastalWatersUntilCartographyIsResearched("Classical era ranged naval unit withRange of 1. Can only operate on Coastal waters until Cartography is researched.", UniqueTarget.Unit),
    Civ6FastMovingIndustrialEraReconUnit("Fast-moving Industrial era recon unit.", UniqueTarget.Unit),
    Civ6CanMoveAfterPerformingAnAction("Can move after performing an action", UniqueTarget.Unit),
    Civ6CanPerformRockAction("Can perform a [rock] action", UniqueTarget.UnitAction),
    Civ6PillagingYields("Pillaging this unit yields approximately [stats]", UniqueTarget.Unit),
    Civ6BombardStrength100("100Bombard Strength", UniqueTarget.Unit),
    Civ6InformationEraSiegeUnitIdealForAttackingCitiesCannotMoveAndAttackOnTheSameTurnUnlessTheyVeEarnedTheExpertCrewPromotion("Information era siege unit, ideal for attacking cities. Cannot move and attack on the same turn unless they've earned the Expert Crew promotion.", UniqueTarget.Unit),
    Civ6FastMovingAncientEraReconUnit("Fast-moving Ancient era recon unit.", UniqueTarget.Unit),
    Civ6MedievalEraSupportUnitWhenAdjacentToACityAttackingMeleeAndAntiCavalryUnitsIgnoreWallsAndImmediatelyAssaultTheCityDoesNotAffectCitiesWithRenaissanceWallsIneffectiveAgainstUrbanDefenses("Medieval era support unit. When adjacent to a city, attacking melee and anti-cavalry units ignore Walls and immediately assault the city. Does not affect cities with Renaissance Walls. Ineffective against Urban Defenses.", UniqueTarget.Unit),
    Civ6MedievalEraReconUnit("Medieval Era recon unit.", UniqueTarget.Unit),
    Civ6WeakAncientEraRangedUnitBetterOnAttackThanDefense("Weak Ancient era ranged unit. Better on attack than defense.", UniqueTarget.Unit),
    Civ6AncientEraAntiCavalryUnit("Ancient era anti-cavalry unit.", UniqueTarget.Unit),
    Civ6AtomicEraReconnaissanceUnitParadropAbilityAllowsMovementFromFriendlyTileToLocation7TilesAway12IfStartingFromAerodromeOrAirstripCanAttackSupportUnitsDirectlyWithoutHavingToEliminateTheCombatUnitFirst("Atomic Era reconnaissance unit. Paradrop ability allows movement from friendly tile to location 7 tiles away (12 if starting from Aerodrome or Airstrip). Can attack support units directly without having to eliminate the combat unit first.", UniqueTarget.Unit),
    Civ6ModernEraRangedRaidingNavalUnitCanOnlyBeSeenByOtherNavalRaidersUnlessAdjacentToItRevealsNavalRaidersWithinSightRange("Modern era ranged raiding naval unit. Can only be seen by other Naval Raiders unless adjacent to it. Reveals Naval Raiders within sight range.", UniqueTarget.Unit),
    Civ6StrongClassicalEraMeleeUnit("Strong Classical era melee unit.", UniqueTarget.Unit),
    Civ6FastMovingModernEraHeavyCavalryUnitThatIgnoresZoneOfControl("Fast-moving Modern era heavy cavalry unit that ignores zone of control.", UniqueTarget.Unit),
    Civ6EstablishesTradeRoute("Establishes a [param] [param] with another city", UniqueTarget.UnitAction),
    Civ6MayActivateATradeRouteAction("May activate a trade route action", UniqueTarget.Unit),
    Civ6BombardStrength45("45Bombard Strength", UniqueTarget.Unit),
    Civ6MedievalEraSiegeUnitIdealForAttackingCitiesCannotMoveAndAttackOnTheSameTurnUnlessTheyVeEarnedTheExpertCrewPromotion("Medieval era siege unit, ideal for attacking cities. Cannot move and attack on the same turn unless they've earned the Expert Crew promotion.", UniqueTarget.Unit),
    Civ6WeakAncientEraMeleeUnit("Weak Ancient era melee unit.", UniqueTarget.Unit),
    Civ6MustBeOnAHill("Must be on a Hill", UniqueTarget.Building),
    Civ6Provides1CultureForEachAdjacentDistrict("Provides +1 Culture for each adjacent district", UniqueTarget.Building),
    Civ6ShowsWhenUnbuildable("Shows when unbuildable", UniqueTarget.Building),
    Civ6BonusStat("+1 [stat]", UniqueTarget.Building),
    Civ6GreatPersonPointsPerTurn("+2 [greatPerson] points per turn", UniqueTarget.Building),
    Civ6CitizenSlot("+1 Citizen slot", UniqueTarget.Building),
    Civ6AirUnitCapacity("Air unit capacity: 2", UniqueTarget.Building),
    Civ6EraScore("EraScore: [amount]", UniqueTarget.Building),
    Civ6MustBeBuiltOnTerrain("Must be built on [terrainName]", UniqueTarget.Building),
    Civ6GovernorXPGain("GovernorXP: [amount]", UniqueTarget.Building),
    Civ6RangedStrike("Ranged Strike", UniqueTarget.Building),
    Civ6GovernorXP("Governor XP: [amount]", UniqueTarget.Building),
    Civ6MustBeBuiltAdjacentToDistrict("Must be built adjacent to [districtName]", UniqueTarget.Building),
    Civ625XPForAllUnits("+25% XP for all units", UniqueTarget.Building),
    Civ6UnitsHeal15HPTurnInCity("Units heal +15 HP/turn in city", UniqueTarget.Building),
    Civ6TradeRouteCapacity("+1 Trade Route capacity", UniqueTarget.Building),
    Civ625XPForUnitsTrainedHere("+25% XP for units trained here", UniqueTarget.Building),
    Civ6Amenity("+1 Amenity", UniqueTarget.Building),
    Civ6HousingBasedOnFreshWaterAvailability("Housing based on fresh water availability", UniqueTarget.Building),
    Civ6ProvidesFreshWaterToCity("Provides fresh water to city", UniqueTarget.Building),
    Civ63GreatMerchantPointsPerTurn("3Great Merchant points per turn.", UniqueTarget.Building),
    Civ6DetectsDisasterRisk("Detects [param] risk tiles in your territory", UniqueTarget.Building),
    Civ6StatInCity("[amount] [stat] [cityFilter]", UniqueTarget.Building),
    Civ6RegionalEffect4ProductionToAllCitiesWithin6Tiles("Regional effect: +4 Production to all cities within 6 tiles", UniqueTarget.Building),
    Civ650ProductionTowardsSettlers("+50% Production towards Settlers", UniqueTarget.Building),
    Civ6MustBeOnCoast("Must be on Coast", UniqueTarget.Building),
    Civ6NavalUnitsHeal20HPTurn("Naval units heal +20 HP/turn", UniqueTarget.Building),
    Civ6RemovesExtraUnhappinessFromAnnexedCities("Removes extra unhappiness from annexed cities", UniqueTarget.Building),
    Civ6PercentProductionTowardsUnits("[amount]% [stat] towards [param] units [cityFilter]", UniqueTarget.Building),
    Civ6RegionalEffect3ProductionToAllCitiesWithin6Tiles("Regional effect: +3 Production to all cities within 6 tiles", UniqueTarget.Building),
    Civ6ABuildingUniqueToAmerica30TourismPressureFromThisCityTowardsOtherCivilizationsWhoReachedTheModernEra("A building unique to America. +30%Tourism pressure from this city towards other civilizations who reached the Modern era.", UniqueTarget.Building),
    Civ6UnhappinessReductionFromPopulation("[amount]% Unhappiness from [populationFilter] [cityFilter]", UniqueTarget.Building),
    Civ61DiplomaticFavorPerTurn("+1 Diplomatic Favor per turn", UniqueTarget.Building),
    Civ6DiplomaticVictoryFromWorldCongress("Diplomatic Victory points from World Congress resolutions [amount]", UniqueTarget.Building),
    Civ6DiplomaticFavor5("DiplomaticFavor: 5", UniqueTarget.Building),
    Civ6FaithMayBeUsedToPurchaseLandAndNavalUnits("Faith may be used to purchase land and naval units", UniqueTarget.Building),
    Civ6MustBeBuiltAdjacentToDistrictSuffix("Must be built adjacent to [districtName] district", UniqueTarget.Building),
    Civ61ProductionPerAdjacentResource("+1 Production per adjacent resource", UniqueTarget.Building),
    Civ62ProductionPerAdjacentAqueductDamCanal("+2 Production per adjacent Aqueduct/Dam/Canal", UniqueTarget.Building),
    Civ62ProductionPerAdjacentCommercialHubMarketBankStockExchange("+2 Production per adjacent Commercial Hub/Market/Bank/Stock Exchange", UniqueTarget.Building),
    Civ6MustBeAdjacentToRiver("Must be adjacent to River", UniqueTarget.Building),
    Civ6RegionalEffect6ProductionToAllCitiesWithin6Tiles("Regional effect: +6 Production to all cities within 6 tiles", UniqueTarget.Building),
    Civ6DiplomaticFavor2("DiplomaticFavor: 2", UniqueTarget.Building),
    Civ6DiplomaticVictoryPointPerTurn("[amount] Diplomatic Victory point per turn", UniqueTarget.Building),
    Civ61SpyCapacity("+1 Spy capacity", UniqueTarget.Building),
    Civ6Requires("Requires [tileFilter] to be built", UniqueTarget.Building, UniqueTarget.District),
    Civ61CultureForEachAdjacentDistrict("+1 Culture for each adjacent district", UniqueTarget.Building),
    Civ6Provides1GreatProphetPointPerTurn("Provides +1 Great Prophet point per turn", UniqueTarget.Building),
    Civ6StatFromTiles("[amount] [stat] from [tileFilter] tiles", UniqueTarget.Building),
    ...
    Civ6DiplomaticFavor3("DiplomaticFavor: 3", UniqueTarget.Building),
    Civ6DiplomaticVictoryPoint("[amount] Diplomatic Victory point [cityFilter]", UniqueTarget.Building),
    Civ6ABuildingUniqueToSweden("A building unique to Sweden.", UniqueTarget.Building),
    Civ6Provides1SlotOfWritingMusicAndAnyTypeOfArt("Provides 1 slot of Writing, Music, and any type of Art.", UniqueTarget.Building),
    Civ6ThisBuildingIsNotMutuallyExclusiveWithOtherBuildingsInGovernmentPlaza("This building is not mutually exclusive with other buildings in Government Plaza.", UniqueTarget.Building),
    Civ61SciencePerCampusAdjacency("+1 Science per Campus adjacency", UniqueTarget.Building),
    Civ62GoldPerAdjacentDistrict("+2 Gold per adjacent district", UniqueTarget.Building),
    Civ6NavalUnitsGet1Movement("Naval units get +1 Movement", UniqueTarget.Building),
    Civ610Science("+10% Science", UniqueTarget.Building),
    Civ6AdjacencyBonus2ScienceForEachAdjacentMine1ForEachAdjacentFarmPlantationLumberMill("Adjacency bonus: +2 Science for each adjacent Mine, +1 for each adjacent Farm/Plantation/Lumber mill", UniqueTarget.Building),
    Civ6BaseScience4("Base Science: +4", UniqueTarget.Building),
    Civ6AllowsPurchasingMissionariesWithFaith("Allows purchasing Missionaries with Faith", UniqueTarget.Building),
    Civ6EnablesUnitConstruction("Enables construction of [unitType] units", UniqueTarget.Building),
    Civ625XPForCavalryUnits("+25% XP for cavalry units", UniqueTarget.Building),
    Civ62Production("+2 Production", UniqueTarget.Building),
    Civ61GoldPerAdjacentRiverTile("+1 Gold per adjacent River tile", UniqueTarget.Building),
    Civ62GoldPerAdjacentHolySiteShrineTemple("+2 Gold per adjacent Holy Site/Shrine/Temple", UniqueTarget.Building),
    Civ620ProductionCostForCommercialHubBuildings("-20% Production cost for Commercial Hub buildings", UniqueTarget.Building),
    Civ61RelicSlot("+1 Relic slot", UniqueTarget.Building),
    Civ6AllowsPurchasingApostlesWithFaith("Allows purchasing Apostles with Faith", UniqueTarget.Building),
    Civ61GreatAdmiralPointPerTurn("1Great Admiral point per turn.", UniqueTarget.Building),
    Civ62GreatScientistPointsPerTurn("2Great Scientist points per turn.", UniqueTarget.Building),
    Civ6CombatStrengthToUnits("+5 Combat Strength to [param] units", UniqueTarget.Building),
    Civ6ReducesEmissions("Reduces [param] emissions by [amount] per turn", UniqueTarget.Building),
    Civ6StatForEachInCity("[amount] [stat] for each [param] in this city", UniqueTarget.Building),
    Civ6MustBeOnTerrainOr("Must be on [terrainName] or [terrainName]", UniqueTarget.Building),
    Civ6StatsFromTilesCityFilter("[amount] [stat] from [tileFilter] tiles [cityFilter]", UniqueTarget.Nation, UniqueTarget.CityState),
    Civ61GreatPersonPointPerTurn("+1 [greatPerson] point per turn", UniqueTarget.Building, UniqueTarget.CityState),
    Civ6RequiresAdjacentToBuild("Requires adjacent [param] to be built", UniqueTarget.Improvement),
    Civ6AmericanUniqueAtomicAirUnit("American unique Atomic era air unit that replaces the Fighter. Gains +5Combat Strength against fighter aircraft, has +2 flight range, and gains [param]% experience", UniqueTarget.Nation),
    Civ6GermanUniqueModernEraNavalUnit("German unique Modern era naval unit that replaces the Submarine. Cheaper to produce, [param] Sight, and +10Combat Strength when fighting on Ocean tiles. Able to reveal other stealthed units", UniqueTarget.Nation),
    Civ6MongoliaBuildingUnique("A building unique to Mongolia. Grants an ability that gives +1Movement to Heavy and Light Cavalry trained in this city. [param]% combat experience for all cavalry and siege class units trained in this city.Strategic Resource Stockpiles increased +10 (on Standard Speed).May not be built in an Encampment district that already has a Barracks.", UniqueTarget.Nation),
    Civ6SpainMissionBuilderUnique("Unlocks the Builder ability to construct a Mission, unique to Spain.+2Faith. +2Faith, +1Production, and +1Food if on a different continent than yourCapital. +1Science for every adjacent Campus and Holy Site district. AdditionalScience once Cultural Heritage is discovered. [param] Loyalty per turn for cities with a Mission improvement adjacent to the City Center and not on your originalCapital's continent.", UniqueTarget.Nation),
    Civ6SwedenOpenAirMuseumBuilderUnique("Unlocks the Builder ability to construct an Open-Air Museum, unique to Sweden.Provides [param] Loyalty per turn in this city. +2Culture and +2Tourism for each type of terrain (Snow, Tundra, Desert, Plains or Grassland) in which at least one Swedish city is founded. One per city. Tiles with Open-Air Museums cannot be swapped.", UniqueTarget.Nation),
    // endregion
