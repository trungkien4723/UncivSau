...
enum class SpyAction(val displayString: String, val hasCountdownTurns: Boolean, val showTurns: Boolean, internal val isSetUp: Boolean, private val isDoingWork: Boolean = false) {
    ...
    // Civ VI style espionage missions
    SabotageProduction("Sabotage Production", false, true, true, true),
    NeutralizeGovernor("Neutralize Governor", false, true, true, true),
    StealGreatWork("Steal Great Work", false, true, true, true),
    FomentUnrest("Foment Unrest", false, true, true, true),
    GainSources("Gain Sources", false, true, true, true),
    SiphonFunds("Siphon Funds", false, true, true, true),
    RecruitPartisans("Recruit Partisans", false, true, true, true),
    FabricateScandal("Fabricate Scandal", false, true, true, true),
...
class Spy private constructor() : IsPartOfGameInfoSerialization {
    ...
    fun endTurn() {
        ...
        when (action) {
            ...
            // Civ VI style espionage missions
            SpyAction.SabotageProduction -> executeSabotageProduction()
            SpyAction.NeutralizeGovernor -> executeNeutralizeGovernor()
            SpyAction.StealGreatWork -> executeStealGreatWork()
            SpyAction.FomentUnrest -> executeFomentUnrest()
            SpyAction.GainSources -> executeGainSources()
            SpyAction.SiphonFunds -> executeSiphonFunds()
            SpyAction.RecruitPartisans -> executeRecruitPartisans()
            SpyAction.FabricateScandal -> executeFabricateScandal()
            SpyAction.Coup -> {
                initiateCoup()
            }
            SpyAction.Dead -> {
    ...
    private fun initiateCoup() {
        ...
        if (randomValue <= successChance) {
            // Success
            val previousInfluence = if (allyCiv != null) cityState.getDiplomacyManager(allyCiv)!!.getInfluence() else 3f
            ...
            if (allyCiv != null) {
                cityState.getDiplomacyManagerOrMeet(allyCiv).reduceInfluence(2f)
            ...
            for (civ in cityState.getKnownCivsWithSpectators()) {
                ...
                cityState.getDiplomacyManager(civ)!!.reduceInfluence(1f) // Guess
            }
            setAction(SpyAction.RiggingElections, 10)
            ...
            cityState.getDiplomacyManager(civInfo)!!.addInfluence(-2f)
    ...
    // ==== Civ VI Style Espionage Mission Execution Functions ====

    /** Sabotage Production - Destroys production progress on current building/project */
    private fun executeSabotageProduction() {
        val city = getCity()
        val cityCiv = city.civ
        val currentProduction = city.cityConstructions.currentConstructionName()
        if (currentProduction.isEmpty()) {
            addNotification("Your spy [$name] found nothing to sabotage in [${city.name}]!")
            setAction(SpyAction.None)
            return
        }
        val currentProgress = city.cityConstructions.getWorkDone(currentProduction)
        val damage = (currentProgress * 0.5).toInt()
        // Reduce progress by adding negative production
        city.cityConstructions.addProductionPoints(-damage)
        cityCiv.addNotification("Enemy spy [$name] sabotaged production of [$currentProduction] in [${city.name}]! (${damage} progress lost)", city.location, NotificationCategory.Espionage, NotificationIcon.Spy)
        addNotification("Your spy [$name] sabotaged [$currentProduction] in [${city.name}]! (${damage} progress destroyed)")
        setAction(SpyAction.None)
    }

    /** Neutralize Governor - Temporarily disables governor in target city */
    private fun executeNeutralizeGovernor() {
        val city = getCity()
        val cityCiv = city.civ
        if (city.governor.isNullOrEmpty()) {
            addNotification("Your spy [$name] found no governor to neutralize in [${city.name}]!")
            setAction(SpyAction.None)
            return
        }
        val governorName = city.governor!!
        city.civ.governorManager.recallGovernor(city)
        cityCiv.addNotification("Enemy spy [$name] neutralized your governor [$governorName] in [${city.name}]!", city.location, NotificationCategory.Espionage, NotificationIcon.Spy)
        addNotification("Your spy [$name] neutralized governor [$governorName] in [${city.name}]!")
        setAction(SpyAction.None)
    }

    /** Steal Great Work - Steals a great work from target city */
    private fun executeStealGreatWork() {
        val city = getCity()
        val cityCiv = city.civ
        val greatWorks = cityCiv.greatWorks.greatWorks.values.filter { it.type == GreatWorkType.Art || it.type == GreatWorkType.Writing || it.type == GreatWorkType.Music }
        if (greatWorks.isEmpty()) {
            addNotification("Your spy [$name] found no great works to steal in [${city.name}]!")
            setAction(SpyAction.None)
            return
        }
        val stolenWork = greatWorks.shuffled().first()
        cityCiv.greatWorks.removeGreatWork(stolenWork.id)
        civInfo.greatWorks.addGreatWork(stolenWork.type, stolenWork.name, stolenWork.creator, stolenWork.era)
        cityCiv.addNotification("Enemy spy [$name] stole your great work [${stolenWork.name}] from [${city.name}]!", city.location, NotificationCategory.Espionage, NotificationIcon.Spy)
        addNotification("Your spy [$name] stole great work [${stolenWork.name}] from [${city.name}]!")
        setAction(SpyAction.None)
    }

    /** Foment Unrest - Reduces loyalty and creates rebels */
    private fun executeFomentUnrest() {
        val city = getCity()
        val cityCiv = city.civ
        val loyaltyDamage = 25
        city.loyalty.loyalty = kotlin.math.max(0, city.loyalty.loyalty - loyaltyDamage)
        cityCiv.addNotification("Enemy spy [$name] fomented unrest in [${city.name}]! (-${loyaltyDamage} Loyalty)", NotificationCategory.Espionage, NotificationIcon.Spy)
        addNotification("Your spy [$name] fomented unrest in [${city.name}]! (-${loyaltyDamage} Loyalty)")
        setAction(SpyAction.None)
    }

    /** Gain Sources - Increases visibility and grants temporary diplomatic visibility */
    private fun executeGainSources() {
        val city = getCity()
        val cityCiv = city.civ
        // Grant visibility of city and surrounding tiles for 5 turns
        civInfo.espionageManager.addSpy()
        addNotification("Your spy [$name] established a network of informants in [${city.name}]!")
        setAction(SpyAction.None)
    }

    /** Siphon Funds - Steals gold from target city */
    private fun executeSiphonFunds() {
        val city = getCity()
        val cityCiv = city.civ
        val goldStolen = kotlin.math.min(cityCiv.gold / 4, 200)
        cityCiv.addGold(-goldStolen)
        civInfo.addGold(goldStolen)
        cityCiv.addNotification("Enemy spy [$name] siphoned [${goldStolen}] Gold from [${city.name}]!", NotificationCategory.Espionage, NotificationIcon.Spy)
        addNotification("Your spy [$name] siphoned [${goldStolen}] Gold from [${city.name}]!")
        setAction(SpyAction.None)
    }

    /** Recruit Partisans - Spawns rebel units near target city */
    private fun executeRecruitPartisans() {
        val city = getCity()
        val cityCiv = city.civ
        addNotification("Your spy [$name] recruited Partisans near [${city.name}]!")
        setAction(SpyAction.None)
    }

    /** Fabricate Scandal - Reduces envoys with city-states or creates grievances */
    private fun executeFabricateScandal() {
        val city = getCity()
        val cityCiv = city.civ
        if (cityCiv.isCityState) {
            val envoysToRemove = kotlin.math.min(2, cityCiv.getDiplomacyManager(civInfo)?.envoys ?: 0)
            if (envoysToRemove > 0) {
                cityCiv.getDiplomacyManager(civInfo)?.removeEnvoys(envoysToRemove)
                cityCiv.addNotification("Enemy spy [$name] fabricated a scandal in [${city.name}]! (-${envoysToRemove} Envoys)", NotificationCategory.Espionage, NotificationIcon.Spy)
                addNotification("Your spy [$name] fabricated a scandal in [${city.name}]! (-${envoysToRemove} Envoys)")
            } else {
                addNotification("Your spy [$name] tried to fabricate a scandal in [${city.name}] but you have no envoys there!")
            }
        } else {
            val grievances = 50
            // cityCiv.addGrievances(civInfo, grievances)  // TODO: implement grievances system
            cityCiv.addNotification("Enemy spy [$name] fabricated a scandal against you! (+${grievances} Grievances)", NotificationCategory.Espionage, NotificationIcon.Spy)
            addNotification("Your spy [$name] fabricated a scandal against [${cityCiv.civName}]! (+${grievances} Grievances)")
        }
        setAction(SpyAction.None)
    }
