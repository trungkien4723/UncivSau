...
class CityScreen(
    ...
    private fun updateLoyaltyAndGovernor() {
        ...
        if (governor != null) {
            ...
            // Promotion button
            val governorManager = city.civ.governorManager
            val currentLevel = governorManager.getGovernorPromotionLevel(city)
            val governorData = city.civ.gameInfo.ruleset.governors[city.governor!!]!!
            if (canChangeState && currentLevel < governorData.promotionObjects.size) {
                loyaltyTable.add("Promote".toTextButton().apply { labelCell.pad(5f) }.onClick {
                    openGovernorPromotionPicker()
                })
            }
    ...
    private fun openGovernorPromotionPicker() {
        val popup = Popup(this)
        popup.addGoodSizedLabel("Choose Promotion").row()
        val governorManager = city.civ.governorManager
        val currentLevel = governorManager.getGovernorPromotionLevel(city)
        val governorData = city.civ.gameInfo.ruleset.governors[city.governor!!]!!
        
        if (currentLevel < governorData.promotionObjects.size) {
            val levelPromotions = governorData.promotionObjects[currentLevel]
            for (promotionUnique in levelPromotions) {
                ...
                val button = promoName.toTextButton().apply {
                    onClick {
                        city.civ.governorManager.promoteGovernor(city)
                        update()
                        game.popScreen()
                    }
                }
                ...
                descLabel.wrap = true
                popup.add(descLabel).padBottom(10f).row()
            }
        }
