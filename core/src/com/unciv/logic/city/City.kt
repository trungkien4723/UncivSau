...
class City : IsPartOfGameInfoSerialization, INamed {
    ...
    @Readonly private fun getGovernorUniques(): Sequence<Unique> {
        ...
        val baseUniques = gov.uniqueObjects.asSequence()
        // Add promotion uniques based on governor level
        val promotionLevel = civ.governorManager.getGovernorPromotionLevel(this)
        if (promotionLevel > 0 && promotionLevel <= gov.promotionObjects.size) {
            val promotionUniques = (0 until promotionLevel).flatMap { lvl -> gov.promotionObjects[lvl].asSequence() }
            return baseUniques + promotionUniques
        }
        return baseUniques
    }
