...
object DeclareWar {
    ...
    private fun notifyOfWar(diplomacyManager: DiplomacyManager, declareWarReason: DeclareWarReason) {
        ...
        when (declareWarReason.warType) {
            WarType.DirectWar,
            WarType.ConquestWar,
            WarType.LiberationWar,
            WarType.HolyWar,
            WarType.ReconquestWar,
            WarType.ColonialWar,
            WarType.ProtectorateWar,
            WarType.RetributionWar -> {
    ...
    private fun changeOpinions(diplomacyManager: DiplomacyManager, declareWarReason: DeclareWarReason) {
        ...
        val casusBelli = declareWarReason.casusBelli

        otherCivDiplomacy.setModifier(DiplomaticModifiers.DeclaredWarOnUs, -20f)
        otherCivDiplomacy.removeModifier(DiplomaticModifiers.ReturnedCapturedUnits)

        // Apply warmongering based on Casus Belli
        val warmongerPenaltyPercent = casusBelli?.warmongerPenaltyPercent ?: 100
        val warmongerBasePenalty = -5f * warmongerPenaltyPercent / 100
            ...
            for (thirdCiv in civInfo.getKnownCivs()) {
                if (!thirdCiv.isAtWarWith(otherCiv)
                    ...
                    thirdCiv.getDiplomacyManager(civInfo)!!
                        .addModifier(DiplomaticModifiers.WarMongerer, warmongerBasePenalty)
                }
            }
        }
...
enum class WarType {
    ...
    // === Casus Belli Types (Civ VI Style) ===
    /** Formal declaration of war with a specific justification - reduces warmonger penalty */
    ConquestWar,
    /** Liberating previously owned cities - no warmonger penalty for those cities */
    LiberationWar,
    /** Holy war against different religion - reduced penalty */
    HolyWar,
    /** Reclaiming former territory - reduced penalty */
    ReconquestWar,
    /** Colonial war against civ on different continent - no penalty for captured cities */
    ColonialWar,
    /** Protectorate war to defend suzerainty - reduced penalty */
    ProtectorateWar,
    /** War of retribution for grievances - moderate penalty */
    RetributionWar
}
 ...
 * @param casusBelli The formal justification for war (Casus Belli) - reduces warmonger penalties
 */
class DeclareWarReason(
    val warType: WarType,
    val allyCiv: Civilization? = null,
    val casusBelli: CasusBelli? = null
)
