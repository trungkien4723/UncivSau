...
enum class DiplomaticModifiers(val text: String) {
    ...
    Delegation("You have a delegation in our capital"),
    ReceivedDelegation("We have a delegation in your capital"),
    SharedDelegation("We have shared delegations"),
...
class DiplomacyManager() : IsPartOfGameInfoSerialization {
    ...
    companion object {
        /** How many envoys a city-state requires to become a friend */
        const val friendThreshold = 1
        /** How many envoys a city-state requires to become an ally */
        const val allyThreshold = 3
        /** How many envoys a city-state requires to become a suzerain */
        const val suzerainThreshold = 6
        /**
    ...
    /** For city-states. Envoys are saved in the CITY STATE -> major civ Diplomacy, NOT in the major civ -> city state diplomacy.
      * Access via getEnvoys() and addEnvoys() unless you know what you're doing.
    ...
    private fun compareRelationshipLevel(level: RelationshipLevel, comparesAs: Int): Boolean {
        ...
        return when(level) {
            RelationshipLevel.Neutral -> comparesAs <= 0 && envoys == 0
            RelationshipLevel.Favorable -> envoys in 1..2
            RelationshipLevel.Friend -> envoys in 1..2
            RelationshipLevel.Ally -> envoys >= allyThreshold && envoys < suzerainThreshold
            RelationshipLevel.Unforgivable -> false // Envoys can't make a CS unforgivable; use military/war for that
            else -> relationshipLevel().compareTo(level).sign == comparesAs
        }
    }
    ...
    fun getTurnsToRelationshipChange(): Int {
        ...
        if (civInfo.isCityState && !otherCiv.isCityState) {
            return when {
                envoys >= suzerainThreshold -> 0 // Suzerain never degrades naturally
                envoys == 0 -> 0
                else -> ceil(envoys.toFloat() / envoysDegradationPerTurn()).toInt() + 1
            }
        }
        return 0
    }
    ...
    @Readonly
    private fun envoysDegradationPerTurn(): Float = 1f

    /** @return Number of envoys this major civ has with this city-state */
    @Readonly
    fun getEnvoys(): Int = envoys

    /** Adds envoys (use positive values) */
    fun addEnvoys(amount: Int) {
        envoys = max(envoys + amount, 0)
        civInfo.cityStateFunctions.updateAllyCivForCityState()
    }

    /** Removes envoys (does nothing if already at 0) */
    fun removeEnvoys(amount: Int) {
        if (envoys <= 0) return
        envoys = max(envoys - amount, 0)
        civInfo.cityStateFunctions.updateAllyCivForCityState()
    }

    /** Sets envoy count directly (use sparingly) */
    ...
    @VisibleForTesting
    fun setInfluenceWithoutSideEffects(amount: Float) { envoys = amount.toInt() }

    /** Adds envoys. The float parameter is converted to Int. */
    fun addInfluence(amount: Float) {
        envoys = max(envoys + amount.toInt(), 0)
        civInfo.cityStateFunctions.updateAllyCivForCityState()
    }

    /** Reduces envoys. Does nothing if already at 0. */
    ...
    /** Sets envoy count directly. Float is converted to Int. */
    fun setInfluence(amount: Float) {
        envoys = amount.toInt().coerceAtLeast(0)
        civInfo.cityStateFunctions.updateAllyCivForCityState()
    }

    /** @return Number of envoys (0 if at war with this city-state) */
    @Readonly
    fun getInfluence(): Float = if (civInfo.isAtWarWith(otherCiv)) -60f else envoys.toFloat()
