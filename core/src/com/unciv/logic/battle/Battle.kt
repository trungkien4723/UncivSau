...
object Battle {
    ...
    fun attack(attacker: ICombatant, defender: ICombatant): DamageDealt {
        ...
        // Check for theological combat (religious unit vs religious unit)
        val isTheologicalCombat = isTheologicalCombat(attacker, defender)
        ...
        val extraRangedAttackDamage = if (isTheologicalCombat) DamageDealt.None else tryExtraRangedAttack(attacker, defender, attackedTile)

        triggerCombatUniques(attacker, defender, attackedTile)

        val damageDealt = takeDamage(attacker, defender, isTheologicalCombat) + extraRangedAttackDamage
        ...
        if (attacker is MapUnitCombatant && defender is MapUnitCombatant){
            val attackAction = if (isTheologicalCombat) CombatAction.TheologicalCombat else CombatAction.Attack
            val defendAction = if (isTheologicalCombat) CombatAction.TheologicalCombat else CombatAction.Defend
            triggerDamageUniquesForUnit(attacker, defender, attackedTile, attackAction)
            if (!attacker.isRanged()) triggerDamageUniquesForUnit(defender, attacker, attackedTile, defendAction)
        }
    ...
    // Check if this is a theological combat (religious unit vs religious unit)
    private fun isTheologicalCombat(attacker: ICombatant, defender: ICombatant): Boolean {
        return attacker is MapUnitCombatant && defender is MapUnitCombatant &&
               attacker.unit.baseUnit.unitType == "Religious" && defender.unit.baseUnit.unitType == "Religious"
    }

    internal fun takeDamage(attacker: ICombatant, defender: ICombatant, isTheologicalCombat: Boolean = false): DamageDealt {
        val combatAction = if (isTheologicalCombat) CombatAction.TheologicalCombat else CombatAction.Attack
        val defenderAction = if (isTheologicalCombat) CombatAction.TheologicalCombat else CombatAction.Defend
        val attackerContext = GameContext(attacker, defender, defender.getTile(), combatAction)
        val defenderContext = GameContext(defender, attacker, defender.getTile(), defenderAction)
        var potentialDamageToDefender = BattleDamage.calculateDamageToDefender(attacker, defender, combatAction = combatAction)
        var potentialDamageToAttacker = BattleDamage.calculateDamageToAttacker(attacker, defender, combatAction = combatAction)
...
if (!otherIsBarbarian && civ.isMajorCiv()) { // Can't get great generals from Barbarians
            ...
            // Great Admiral points for naval combat
            var greatAdmiralUnits = civ.gameInfo.ruleset.greatAdmiralUnits
                .filter { it.hasUnique(UniqueType.GreatPersonFromCombat, gameContext) &&
                        it.getRejectionReasons(civ).none { reason ->
                            !reason.isConstructionRejection() &&
                                    !reason.techPolicyEraWonderRequirements() }
                    }.asSequence()
            if (civ.gameInfo.ruleset.greatAdmiralUnits.isEmpty() &&
                civ.gameInfo.ruleset.units["Great Admiral"] != null)
                greatAdmiralUnits += civ.gameInfo.ruleset.units["Great Admiral"]!!

            for (unit in greatAdmiralUnits) {
                val greatAdmiralPointsBonus = thisCombatant
                    .getMatchingUniques(UniqueType.GreatPersonEarnedFaster, gameContext, true)
                    .filter { unit.matchesFilter(it.params[0], gameContext) }
                    .sumOf { it.params[1].toDouble() }
                val greatAdmiralPointsModifier = 1.0 + greatAdmiralPointsBonus / 100

                val greatAdmiralPointsGained = (xpGained * greatAdmiralPointsModifier).toInt()
                civ.greatPeople.greatAdmiralPointsCounter[unit.name] += greatAdmiralPointsGained
            }
        }
